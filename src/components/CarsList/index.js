...
function CarsList(available, year, capacity) {
  const { getCarsResult, getCarsLoading, getCarsError } = useSelector(
  ...
  return (
    <div id="cars-container">
      {getCarsResult ? (
        getCarsResult.map((item) => (
          <div className="border rounded p-3 m-2 car-item">
            <div className="d-flex flex-column justify-content-between h-100">
              <img src={item.image} alt={item.manufacture} className="carimg" />
              <div>
                <p className="nameimg">
                  {item.manufacture} / {item.model}
                </p>
                <p>
                  <b>Rp {parseRupiah(item.rentPerDay)}/hari</b>
                </p>
                <p>
        ...
        <div>
          <div className="no-data text-center alert alert-danger">
            Tidak Ada Data
          </div>
        </div>
      )}
